...
class AdaptiveAugment:
    ...
    def tune(self, real_pred):
        # print('in tune fct: ', torch.sign(real_pred), self.ada_aug_buf)
...
def try_sample_affine_and_pad(img, p, kernel_size, G=None, seg=None):
    ...
    seg_pad = None
    if seg!=None:
        seg_pad = F.pad(seg, (pad_x1, pad_x2, pad_y1, pad_y2), mode="reflect")
        # print('size of seg_pad: ', seg_pad.size(), torch.min(seg_pad), torch.max(seg_pad))

    # print('before and after padding :',img.size(), img_pad.size(), pad_x1, pad_x2, pad_y1, pad_y2)

    return img_pad, seg_pad, G_try, (pad_x1, pad_x2, pad_y1, pad_y2)
...
def random_apply_affine(img, p, G=None, antialiasing_kernel=SYM6, seg=None):
    ...
    img_pad, seg_pad, G, (pad_x1, pad_x2, pad_y1, pad_y2) = try_sample_affine_and_pad(
        img, p, len_k, G, seg=seg
    )
    ...
    # print('before upf :', torch.min(img_pad), torch.max(img_pad))
    img_2x = upfirdn2d(img_pad, kernel.unsqueeze(0), up=(2, 1), pad=(*up_pad[:2], 0, 0))
    if seg!=None:
        seg_2x = upfirdn2d(seg_pad, kernel.unsqueeze(0), up=(2, 1), pad=(*up_pad[:2], 0, 0))
        # print('after upf :', seg_2x.size(), torch.min(seg_2x), torch.max(seg_2x))

    img_2x = upfirdn2d(img_2x, kernel.unsqueeze(1), up=(1, 2), pad=(0, 0, *up_pad[2:]))
    if seg!=None:
        seg_2x = upfirdn2d(seg_2x, kernel.unsqueeze(1), up=(1, 2), pad=(0, 0, *up_pad[2:]))
        # print('after upf 2:', seg_2x.size(), torch.min(seg_2x), torch.max(seg_2x))
    ...
    if seg!=None:
        _, channel_seg, _, _ = seg.shape
        shape_seg = (batch_size, channel_seg, (height + pad_k * 2) * 2, (width + pad_k * 2) * 2)
        grid_seg = F.affine_grid(G_inv[:, :2, :].to(img_2x), shape_seg, align_corners=False)
        seg_affine = grid_sample(seg_2x, grid_seg)
    ...
    if seg!=None:
        seg_down = upfirdn2d(
            seg_affine, kernel_flip.unsqueeze(0), down=(2, 1), pad=(*down_pad[:2], 0, 0)
        )
        seg_down = upfirdn2d(
            seg_down, kernel_flip.unsqueeze(1), down=(1, 2), pad=(0, 0, *down_pad[2:])
        )
        # print('size/range of seg_down :', seg_down.size(), torch.min(seg_down), torch.max(seg_down))

    return img_down, seg_down, G
...
def augment(img, p, transform_matrix=(None, None), seg=None):
    img, seg_a, G = random_apply_affine(img, p, transform_matrix[0], seg = seg)
    # print('after affine transf :', torch.min(img), torch.max(img))
    img, C = random_apply_color(img, p, transform_matrix[1])
    # print('after color transf :', torch.min(img), torch.max(img))
