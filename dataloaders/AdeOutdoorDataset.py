...
class AdeOutdoorDataset(torch.utils.data.Dataset):
    ...
    def list_images(self):
        mode = "validation" if self.for_metrics else "training"
        self.opt.dataroot = '/checkpoint/marlenec/ADE_outdoor'
        path_img = os.path.join(self.opt.dataroot, "images", 'training')
        path_lab = os.path.join(self.opt.dataroot, "annotations", 'training')
        ...
        if self.opt.perctg_train_data != 1. and mode =='training':
