...
def get_start_iters(start_iter, dataset_size):
    ...
    print(start_iter, dataset_size, start_epoch)
...
def update_EMA(model, cur_iter, dataloader, opt, force_run_stats=False):
    ...
    with torch.no_grad():
        for key in model.module.netEMA.state_dict():
            # if opt.transfer_l!='' and opt.train_index_gabeta=='' and not 'mlp_shared' in key:
            #     continue
            # if opt.transfer_l != '' and opt.train_index_gabeta == '0,1,2,3,4,5' and not 'mlp' in key and not 'fc' in key:
            #     continue
            # if 'running' in key:
            #     if not torch.all(model.module.netEMA.state_dict()[key].data==model.module.netG.state_dict()[key].data):
            #         print('not equal for EMA update :', key)
            # if not torch.all(model.module.netEMA.state_dict()[key].data==model.module.netG.state_dict()[key].data):
            #     print('not equal for EMA update :', key)

                # print(f'for key : {key}, equality is {torch.all(model.module.netEMA.state_dict()[key].data==model.module.netG.state_dict()[key].data)}')
            # print('in EMA update: ', key)
            # if 'mat_conf' in key:
            #     print('VALUE OF KEY IN UPDATE EMA: ', model.module.netEMA.state_dict()[key].data[0], model.module.netG.state_dict()[key].data[0])
    ...
    if condition_run_stats:
        with torch.no_grad():
            ...
            for i, data_i in enumerate(dataloader):
                ...
                fake = model.module.netEMA(label[:10])
                num_upd += 1
                if num_upd > 50:
                    break
...
class image_saver():
    ...
    def visualize_batch(self, model, image, label, cur_iter):
        ...
        with torch.no_grad():
            model.eval()
            fake = model.module.netG(label[:min(self.rows * self.cols, len(label))])
            print('in save', fake.size())
            self.save_images(fake, "fake", cur_iter)
            model.train()
            if not self.opt.no_EMA:
                model.eval()
                fake = model.module.netEMA(label[:min(self.rows * self.cols, len(label))])
